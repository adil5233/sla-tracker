...
export function calculateSLA(createdAt: Date, priority: Priority): Date {
  ...
  while (remainingHours > 0) {
    ...
    startOfWorkDay.setHours(Math.floor(WORKING_HOURS.start), (WORKING_HOURS.start % 1) * 60, 0, 0);
    
    const endOfWorkDay = new Date(currentDate);
    endOfWorkDay.setHours(Math.floor(WORKING_HOURS.end), (WORKING_HOURS.end % 1) * 60, 0, 0);
    ...
    if (timeLeftToday > 0) {
      ...
      currentDate = new Date(currentDate.getTime() + timeLeftToday * 60 * 60 * 1000);
    } else {
      currentDate = nextWorkingDay(currentDate);
    }
  }
...
export function nextWorkingDay(date: Date): Date {
  ...
  newDate.setHours(Math.floor(WORKING_HOURS.start), (WORKING_HOURS.start % 1) * 60, 0, 0);
...
export function getSLAStatus(now: Date, slaTime: Date, priority: Priority): SLAStatus {
